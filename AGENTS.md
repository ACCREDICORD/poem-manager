...
9. **格律校验（M7）**：韵书字表在 `rhyme_dict` 表（四部韵书，启动时从 `backend/scripts/rhyme_seed.json` 幂等入库；词谱从 `tunes_seed.json` 灌入 `templates`，20 个知名词牌另有正格+2 变格在 `famous_tunes_seed.json`）。校验逻辑在 `app/rhyme.py`：诗按黏对规则推导起收式、只严查二四六；词逐句对照词谱（正文按逗号分句、跳过标题行），`/api/rhyme/check` 自动比对正格+全部变格并选最贴合格。**注意：评分（scoring）已与格律解耦——评委提示词不引用词谱/韵书内容**（因部分变格库中可能缺失，注入会误导打分）；格律只由用户主动点「格律校验」时使用。重新抓取数据：`python scripts/scrape_rhyme.py` / `scrape_famous_tunes.py`（注意礼貌限速与页面结构变化）。
